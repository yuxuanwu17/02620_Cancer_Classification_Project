...
data <- fread("TCGA-LUAD.htseq_counts.tsv", sep = "\t", header = TRUE)
...
conditions <- labelToFacor(colnames(data_cleaned), "11", "01", "02")
...
# read fpkm data
data_fpkm <- fread("TCGA-LUAD.htseq_fpkm.tsv", sep = "\t", header = TRUE)
...
LUAD_final <- data_fpkm[rownames(data_fpkm)%in% features,
                        colnames(data_fpkm)%in% samples]
# write to file
write.table(LUAD_final, "LUAD.txt", sep = "\t", 
